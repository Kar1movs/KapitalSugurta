...
/**************************************************************/
if ($(".star-tag-body").length) {
    $(".star-tag-body").hide();
    const myTimeout = setTimeout(myGreeting, 60000);

    function myGreeting() {
        if (window.location.pathname != '/site/voting') {
            $(".star-tag-body").fadeIn();
        }
    }
    ...
    $(".star-tag-body-general-closer").click(function closeContent() {
        $(".star-tag-body").fadeOut();
    });
}
/* ********************************************************** */
/* ########################################################## */
/* Star toggle size incremine or decremine ****************** */
if ($(".mode__star").length) {
    let starMode = localStorage.getItem("starMode");
    const starModeToggle = document.querySelector(".mode__star");
    /*let starSvgNone = document.querySelectorAll(".star-svg-none");*/
    let starSaveToggle = document.querySelector(".starMode input[id='starBtn']");
    let starSentButton = document.getElementById("sentStarComment");
    let starTagBody = document.querySelector(".star-tag-body");
// let socialPositionStar = document.querySelector('.social-position-star');
    let widthRightNowStar = window.innerWidth;
    let descTopWidthStar = 1024;


    const enableStarMode = () => {
        starModeToggle.classList.add("active");
        starTagBody.classList.add("active");
        starSaveToggle.setAttribute("checked", "true");

        // for (let i = 0; i < svgNone.length; i++) {
        //     const element = svgNone[i];
        //     element.style.display = "none";
        // }
        localStorage.setItem("starMode", "enabled");
    };
    const disableStarMode = () => {
        starModeToggle.classList.remove("active");
        starTagBody.classList.remove("active");
        starSaveToggle.setAttribute("checked", "false");
        ...
        localStorage.setItem("starMode", null);
    };
    if (starMode === 'enabled') {
        enableStarMode();

    }
    if ((widthRightNowStar >= descTopWidthStar) && (starMode === null)) {
        enableStarMode();
    }
    starModeToggle.addEventListener("click", () => {

        starMode = localStorage.getItem("starMode");
...
if ($(".introSwiper").length) {
  ...
  <svg class="swiper-progress" width="31" height="31"><circle class="circle-origin" r="15" cx="15.5" cy="15.5"></circle></svg><span class="d-flex align-content-center justify-content-center"><img class="d-flex align-content-center justify-content-center" src="/themes/v1/images/svg/icons/dot.svg"></span>
</button>
      `;
            },
...
$(document).ready(function () {
    var open = 0;
    $('.search-box-general .search-box .tt-scrollable-menu').hide();
    $('.search-btn').on("click", function () {
        ...
        if (!open) {
            $('.search-box-general .search-box .tt-scrollable-menu').fadeIn();
            open = 1;
            ...
            if ($('.backdrop').length) {
                $('.backdrop').on("click", function () {
                    ...
                    $('.search-box-general .search-box .tt-scrollable-menu').fadeOut();
                    console.log('OFF IN');
...
if ($('body').find('#contact-insured_date')) {
    var startDate = $('#contact-insured_date');
    startDate.datepicker({
        autoHide: true,
        startDate: new Date((new Date()).valueOf() - 1000 * 3600 * 24 * 40),
        minDate: new Date((new Date()).valueOf() - 1000 * 3600 * 24 * 40),
